...
class m3monitoring extends StatefulWidget {
  static const nameRoute = '/m3monitoring ';
  const m3monitoring(
    String p, {
    super.key,
  });
  ...
  @override
  State<m3monitoring> createState() => _m3monitoringState();
}
...
class _m3monitoringState extends State<m3monitoring> {
  ...
  readStock getstock = readStock();
  Future<void> stockData() async {
    stockList = await getstock.getStock(3);
  ...
  Future<void> QualityData() async {
    QList = await Quality.getQualityM(3, "$tipe");
  ...
  Future<void> Avaidata() async {
    AList = await Availability.availabilityM(3);
    streamTime.add(AList);
  }

  //Life Time
  Future<void> lifetime() async {
    getOneLT().getOne(3).then((value) {
        timevalue = value;
    });
  }
  ...
  @override
  void initState() {
    getLatestParamM3.getTipe().then((value) {
      setState(() {
        tipe = value!;
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: DefaultTabController(
        length: m3monitoring.myTab.length,
        child: Scaffold(
          appBar: AppBar(
            title: Text(
          ...
          body: Container(
            ...
            child: TabBarView(
                children: [Production(context), m3pressure(), m3energy()]),
          ),
        ),
      ),
    );
