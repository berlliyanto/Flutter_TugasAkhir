...
class m4oee extends StatefulWidget {
  static const nameRoute = '/m4oee';
  const m4oee(String an, {super.key});

  @override
  State<m4oee> createState() => _m4oeeState();
}

class _m4oeeState extends State<m4oee> {
  late Timer timer;
  String? tipe;
  ...
  Future<void> QualityData() async {
    QList = await Quality.getQualityM(4, "$tipe");
  ...
  Future<void> Avaidata() async {
    AList = await Availability.availabilityM(4);
  ...
  Future<void> Perdata() async {
    PList = await Performance.getPerform(4);
  ...
  Future<void> OEEdata() async {
    OEEList = await OEE.getResult(4);
    streamOEE.add(OEEList);
  }

  @override
  void initState() {
    getLatestParamM4.getTipe().then((value) {
      setState(() {
        tipe = value!;
      });
    });
